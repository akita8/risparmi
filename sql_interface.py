...
# temporaneo
    def import_stock_csv(self):
        ...
        #stock
        if tipo=='stock':
            columns_names=['id', 'symbol', 'denomination', 'market', 'sector', 'currency',
                           'isin', 'nation', 'transaction', 'tax_on_purchase_percentage',
                           'account', 'quantity', 'buy_sell_price', 'dividend', 'commission',
                           'tax_percentage', 'exchange_rate', 'owner', 'date_of_transaction']
            commit_list=[Stock(**dict(zip(columns_names, el))) for el in rows_cleaned]
